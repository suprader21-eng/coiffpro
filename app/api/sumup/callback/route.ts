...
export async function GET(req: NextRequest) {
  ...
  const merchantCode = merchantInfo?.merchant_profile?.merchant_code

  const db = getSupabaseAdmin()
  await db.from('salons').update({
    sumup_access_token: tokens.access_token,
    sumup_refresh_token: tokens.refresh_token,
    sumup_merchant_code: merchantCode,
    sumup_token_expires_at: new Date(Date.now() + tokens.expires_in * 1000).toISOString(),
  }).eq('id', salonId)
